...
async function showNftModal(slug) {
  ...
  // جلب البيانات
  try {
    const res = await fetch(`https://web-production-7ecf2.up.railway.app/nft-image-data/${slug}`, { signal: AbortSignal.timeout(8000) });
    if (res.ok) {
...
function _initTcUI() {
  ...
  _tcUI = new window.TON_CONNECT_UI.TonConnectUI({
    manifestUrl: 'https://web-production-7ecf2.up.railway.app/tonconnect-manifest.json',
    enableAndroidBackHandler: true,
    actionsConfiguration: {
      twaReturnUrl: 'https://web-production-7ecf2.up.railway.app',
    },
  });
